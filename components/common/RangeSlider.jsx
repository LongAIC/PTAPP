...
const RangeSlider = ({
  ...
  const position = useSharedValue(
    ((initialMin - min) / step) * (sliderWidth / ((max - min) / step))
  );
  const position2 = useSharedValue(
    ((initialMax - min) / step) * (sliderWidth / ((max - min) / step))
  );
  ...
  useEffect(() => {
    position.value = ((initialMin - min) / step) * (sliderWidth / ((max - min) / step));
    position2.value = ((initialMax - min) / step) * (sliderWidth / ((max - min) / step));
  ...
  const pan2 = Gesture.Pan()
    ...
    .onUpdate(e => {
      ...
      } else if (context2.value + e.translationX < 0) {
        position2.value = 0;
